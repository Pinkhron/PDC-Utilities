...
class CCCreateConfirmation(discord.ui.View):
    ...
    @discord.ui.button(emoji=':white_check_mark:', label='Confirm', style=discord.ButtonStyle.green)
    ...
    @discord.ui.button(emoji=':x:', label='Cancel', style=discord.ButtonStyle.gray)
    ...
    @discord.ui.button(emoji=':scroll:', label='CC Help & Guidelines', style=discord.ButtonStyle.blurple)
    async def _help(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            embed=discord.Embed(
                title=':scroll: CC Help & Guidelines',
                description='**Help:**\n\n'
                            'You will have a private channel (like a private thread). You will also have'
...
class CustomChannel(commands.GroupCog, name='cc'):
    ...
    async def create(self, interaction: discord.Interaction):
        ...
        if not interaction.channel_id == Data.TXT_BOTS:
            await interaction.response.send_message(
                content=f':x: You cannot use this (/) command here. Goto <#{Data.TXT_BOTS}>',
                ephemeral=True
            )
            return
...
def setup(bot: commands.Bot):
    await bot.add_cog(CustomChannel(bot))
